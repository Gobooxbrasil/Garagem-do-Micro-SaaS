...
import { Share2, Check, Copy, Link as LinkIcon, Twitter, Linkedin, MessageCircle, Send } from 'lucide-react';
...
const ShareButton: React.FC<ShareButtonProps> = ({ idea, variant = 'card' }) => {
  ...
  const handleShare = (platform: 'whatsapp' | 'telegram' | 'twitter' | 'linkedin') => {
        ...
        {isOpen && (
          <div className="absolute right-0 bottom-full mb-2 w-56 bg-white rounded-xl shadow-xl border border-gray-100 z-50 p-2 animate-in fade-in zoom-in-95">
            <div className="grid grid-cols-4 gap-1 mb-2">
              <button onClick={() => handleShare('whatsapp')} className="flex-1 bg-green-50 hover:bg-green-100 text-green-600 p-2 rounded-lg transition-colors flex justify-center" title="WhatsApp"><MessageCircle className="w-4 h-4" /></button>
              <button onClick={() => handleShare('telegram')} className="flex-1 bg-blue-50 hover:bg-blue-100 text-blue-500 p-2 rounded-lg transition-colors flex justify-center" title="Telegram"><Send className="w-4 h-4" /></button>
              <button onClick={() => handleShare('twitter')} className="flex-1 bg-sky-50 hover:bg-sky-100 text-sky-500 p-2 rounded-lg transition-colors flex justify-center" title="Twitter"><Twitter className="w-4 h-4" /></button>
              <button onClick={() => handleShare('linkedin')} className="flex-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 p-2 rounded-lg transition-colors flex justify-center" title="LinkedIn"><Linkedin className="w-4 h-4" /></button>
            </div>
            <button
              onClick={handleCopy}
        ...
        <div className="absolute top-full left-0 mt-2 w-full bg-white rounded-xl shadow-xl border border-gray-100 z-50 p-3 animate-in fade-in zoom-in-95">
          <div className="grid grid-cols-4 gap-2 mb-3">
            <button onClick={() => handleShare('whatsapp')} className="flex flex-col items-center gap-1 p-2 hover:bg-gray-50 rounded-lg text-xs text-gray-600">
              <div className="w-8 h-8 rounded-full bg-green-100 flex items-center justify-center text-green-600"><MessageCircle className="w-4 h-4" /></div>
              WhatsApp
            </button>
            <button onClick={() => handleShare('telegram')} className="flex flex-col items-center gap-1 p-2 hover:bg-gray-50 rounded-lg text-xs text-gray-600">
              <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-500"><Send className="w-4 h-4" /></div>
              Telegram
            </button>
            ...
            <button onClick={() => handleShare('linkedin')} className="flex flex-col items-center gap-1 p-2 hover:bg-gray-50 rounded-lg text-xs text-gray-600">
              <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600"><Linkedin className="w-4 h-4" /></div>
              LinkedIn
            </button>
          </div>
          <button
