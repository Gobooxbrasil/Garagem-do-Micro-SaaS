...
import { Heart, ExternalLink, User, Flame, Rocket, Youtube, Target, Megaphone } from 'lucide-react';
...
export const ShowroomCard: React.FC<ShowroomCardProps> = ({ project, onClick, onToggleFavorite, onVote }) => {
  ...
  const hasVideo = !!project.showroom_video_url;
  ...
  return (
    <div className="group relative bg-white rounded-3xl overflow-hidden shadow-soft border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col h-full">
        {/* Cover Image */}
        <div className="h-48 w-full relative overflow-hidden bg-gray-50 cursor-pointer" onClick={() => onClick(project)}>
            {hasImage ? (
                <img 
                    src={hasImage} 
                    alt={project.title} 
            ...
            {hasVideo && (
                <div className="absolute bottom-4 right-4 bg-red-600/90 backdrop-blur-sm text-white p-1.5 rounded-lg shadow-sm">
        ...
        <div className="p-6 flex flex-col flex-grow">
            ...
            <p className="text-sm text-gray-500 leading-relaxed line-clamp-2 mb-4 font-light">
